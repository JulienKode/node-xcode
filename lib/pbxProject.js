...
pbxProject.prototype.getTargets = function() {

    // Get targets UUID
    var targetsUuid = this.getFirstProject()['firstProject']['targets'].map((target) => item.value);

    // Get all pbxNativeTarget, and returns targets data
    return targetsUuid.map((uuid) => ({uuid: uuid, target: this.pbxNativeTargetSection()[uuid]}));
}
